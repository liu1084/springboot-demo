...
public class VideoServiceImpl implements IVideoService {
	...
	@Value("${video.file.nameFormatter}")
	...
	private Map<String, String> mimeTypes = new ConcurrentHashMap<>();

	@PostConstruct
	public void init() {
		mimeTypes.put("mp4", "video/quicktime");
		mimeTypes.put("rmvb", "audio/x-pn-realaudio");
	...
	private void handleUploadBlock(FileBlockInfo fileBlockInfo) throws IOException {
		...
		//大文件被切割成小文件的每个小文件的命名
		String destName = String.format(filenameFormatter, fileName, fileBlockInfo.getIndex());
		Path destPath = Paths.get(destName);
		try (BufferedInputStream bufferedInputStream = new BufferedInputStream(fileBlockInfo.getUploadFile().getInputStream());) {
	...
	private void addUploadBlockToDB(FileBlockInfo fileBlockInfo) {
		...
		pubVideoBlockInfo.setBlockMd5(fileBlockInfo.getBlockMD5());
		pubVideoBlockInfo.setVideoId(0L); //在文件上传完成后, 会更新
	...
	/**
	 * 检查文件块是否已经存在
	 * 如果文件不存在或者文件的MD5不相同, 返回false
	 *
	 * @param fileBlockInfo
	 * @return
	 */
	private boolean checkBlockExist(FileBlockInfo fileBlockInfo) throws IOException, NoSuchAlgorithmException {
		String target = uploadLocation + "/" + fileBlockInfo.getMd5();
		Path path = Paths.get(target);
		if (!Files.exists(path)) {
			return false;
		...
		String destFilename = String.format(filenameFormatter, target + "/" + fileBlockInfo.getMd5(), fileBlockInfo.getIndex());
		path = Paths.get(destFilename);
			...
			if (pubVideoInfo.getAchTag()) {
				...
				if (parts == fileBlockInfo.getBlockCount()) {
					...
					if (!checkBlockExist(fileBlockInfo)) {
						...
						Path path = Paths.get(target);
						String currentBlockFileMD5 = getFileMD5(Files.newInputStream(path));
	...
	public void mergeChunks(VideoMergeReqDTO videoMergeReqDTO) throws IOException {
		//TODO!
		videoMergeReqDTO.setUsername("zff");
		PubVideoInfo isExist = pubVideoInfoMapper.queryVideoInfoByMD5(videoMergeReqDTO.getMd5());
		if (isExist == null) {
			handleUploadedFile(videoMergeReqDTO);
		}
		Path path = Paths.get(uploadLocation + "/" + videoMergeReqDTO.getMd5() + "/" + videoMergeReqDTO.getMd5() + getFileExtension(videoMergeReqDTO.getFileMimeType()));
		if (!Files.exists(path)) {
	...
	private String getFileExtension(String fileMimeType) {
		//TODO!
//		Predicate<Map.Entry<String, String>> predicate = stringStringEntry -> {
//			return stringStringEntry.getValue().matches(fileMimeType);
//		};
...
//				.collect(Collectors.joining());
	...
	private void handleUploadedFile(VideoMergeReqDTO videoMergeReqDTO) {
		...
		pubVideoInfo.setOriginName(videoMergeReqDTO.getOriginFileName());
		int current = Instant.now().getNano();
		pubVideoInfo.setHashName(String.valueOf(current));
		long videoId = pubVideoInfoMapper.insertPubVideoInfo(pubVideoInfo);
		// 更新videoId
		pubVideoBlockInfoMapper.updatePubVideoBlockInfoByMD5(videoMergeReqDTO.getMd5(), videoId);
	}
...
//        //获取hash值
//        byte[] hashBytes = md.digest(inputStream);
//        //转换hash值为十六进制值
